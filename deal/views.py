...
def create_deal(request):
    ...
    if request.method == 'POST':
        ...
        form_compute = DealComputeDealForm()
        form_asset = DealAssetTypeForm()
    return render(request, 'deal/Views/deal-form.html', {'deal': form_deal,'adress':form_adress, 'compute':form_compute, 'asset':form_asset})
get = None
...
def deal_diaply_on_search(request):
    ...
    if request.method == 'POST':
        ...
        if form_adress.is_valid() and form_asset.is_valid() and form_compute.is_valid() and form_deal.is_valid():
            ...
            logger.debug("Address, asset types, compte parameters was saved.deal was created and Updated on request")
        else:
            logger.debug("Form was invalid. Nothing will be saved.")
...
def deal_diaplay_specific_search_id(request, pk):
    ...
    model classes.The class will also be added to the user so as to get deal per user 
    """
    logger.debug("Display deal {0}".format(pk))
    deal = Deals.objects.get(pk = pk) 
    address = Adress.objects.get(pk = pk) 
    asset = AssetTypes.objects.get(pk = pk) 
    compute = ComputeDeals.objects.get(pk = pk) 
    ...
    return render(request, 'deal/Views/display_specific.html', {'deal':deal,'deal_form': form_deal,'adress':form_adress, 'compute':form_compute, 'asset':form_asset})
